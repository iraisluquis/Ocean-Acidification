...
bats_co2sys_surf %>%
  ...
  geom_smooth(method="lm", size = 1.5, color = "#fbb4ae") +
...
bats_co2sys_surf_pred %>% 
  ...
  labs(x = "Years", y = "pCO2 (µatm)", title = "Prediction surface pCO2 vs years",
       subtitle = "Data source: BATS.")+
  scale_x_continuous(limits = c(1988,2021))+
  theme_classic()+
...
The model interpretation shows that pCO2 does increase with time. There is a a consistent, detectable (ANOVA, p<0.001) annual increase in pCO2 by 1.86+-0.00 µatm/year. This could be attributed to the changes between seasons in surface ocean pCO2 at BATS with higher pCO2 observed in late summer to early fall and lower pCO2 observed in late winter and early spring. 

Figure 1: The points represent de data of pCO2 from 1988 to 2021, the line would be the pCO2 model and the thickness of the line covers the confidence interval of 95%
...
bats_co2sys_surf %>%
  ...
  geom_smooth(method="lm") +
  scale_x_continuous(limits = c(1988,2021))+
  labs(x = "Years", y = "Ωar", title = "BATS Surface OmegaAragonite vs years",
       subtitle = "Data source: BATS.")+
  theme_classic()+
  theme(plot.title = element_text(hjust = 0.5))
...
bats_co2sys_surf_pred %>% 
  ...
  geom_line(aes(decy,fit),color="red")+
  geom_ribbon(aes(x=decy,ymin=lwr,ymax=upr),alpha=0.2,fill="red")+
  labs(x = "Years", y = "pCO2", title = "Prediction surface OmegaAragonite vs years",
       subtitle = "Data source: BATS")+
  scale_x_continuous(limits = c(1988,2021))+
  theme_classic()+
    theme(plot.title = element_text(hjust = 0.5))
