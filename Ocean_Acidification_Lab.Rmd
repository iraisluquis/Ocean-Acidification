...
Figure 1: The points represent the data of pCO2 from 1988 to 2021, the line would be the pCO2 model and the shaded region of the line covers the confidence interval of 95%. 

In the graph we can see fluctuations on the surface pCO2 concentration on the same year, this changes can be attributed to seasonal changes at BATS. During summer to fall there is higher surface pCO2 and during winter to spring there is a lower surface pCO2 concentration. The model interpretation shows that pCO2 does increase with time based on the linear regression and ANOVA (shows variability). Where the p-value was 2.2e-16,there is a consistent, detectable (ANOVA, p<0.001) annual increase in pCO2 by 1.86+-0.00 µatm/year.
...
Figure 2: The points represent the data of pH from 1988 to 2021, the line would be the pH model and the shaded region of the line covers the confidence interval of (??????) 

In the graph we can see fluctuations on the surface pH concentration on the same year, this changes can be attributed to seasonal changes at BATS. During winter months the pH is lower and it increases in the summer. 

The model interpretation shows that pH decrease with time base on the linear regression and ANOVA. Where the p-value was -1.793e-03 which show a decrease on ocean pH. This could be attributed to ocean acidification due to climate change 

#CHATThe information provided shows the result of a linear regression analysis conducted to determine if surface ocean seawater pH is decreasing over time. The model indicates that there is a statistically significant negative relationship between pH and decadal year (decy), as evidenced by the coefficient estimate for decy (-1.793e-03) and its associated p-value (<2e-16). This indicates that as time passes, surface ocean seawater pH is decreasing.

# Where the p-value was 2.2e-16,there is a consistent, detectable (ANOVA, p<0.001) annual increase in pCO2 by 1.86+-0.00 µatm/year.
...
#Adding the prediction line of OemgaAragonite_model to ggplot 
#This predictions could be used to merge the bats_co2sys_surf with the bats_co2sys_surf_pred. We can make with this a two variable plot with the prediction model giving a shaded region of 95% of confidence around the model line.
